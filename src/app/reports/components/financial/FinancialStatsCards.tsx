...
import { IFinancialMainReport } from "@/types/report/financial";

interface FinancialStatsCardsProps {
  stats: IFinancialMainReport;
}
...
export default function FinancialStatsCards({ stats }: FinancialStatsCardsProps) {
  ...
  // می‌توان trend را براساس نیاز حساب کرد، فعلاً مقدار نمونه گذاشته شده
  const trends = {
    totalSalesPrice: 5.2,
    productCost: -2.1,
    totalExpense: 1.5,
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      <StatCard
        title="فروش کل"
        value={report.total_sales_price.toLocaleString()}
        subtitle="افغانی"
        icon={DollarSign}
        trend={trends.totalSalesPrice}
        color="green"
      />
      <StatCard
        title="هزینه محصول"
        value={report.product_cost.toLocaleString()}
        subtitle="افغانی"
        icon={Archive}
        trend={trends.productCost}
        color="orange"
      />
      <StatCard
        title="هزینه کل"
        value={report.total_expense.toLocaleString()}
        subtitle="افغانی"
        icon={BarChart3}
        trend={trends.totalExpense}
        color="purple"
      />
      <StatCard
        title="سود خالص"
        value={report.net_income.toLocaleString()}
        subtitle="افغانی"
        icon={Receipt}
        trend={trends.netIncome}
        color="blue"
      />
    </div>
  );
