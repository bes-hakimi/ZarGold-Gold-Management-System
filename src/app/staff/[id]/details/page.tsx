...
import { User, Phone, MapPin, Calendar, Circle, RefreshCw, Mail } from "lucide-react";
...
export default function StaffDetailsPage() {
  ...
  const { data, isLoading, error } = useApiGet<IUserDetailsResponse>(
    `user-${staffId}`,
  ...
  const handleEdit = () => router.push(`/compnay/${staffId}/edit`);

  const handleDeleteConfirm = async () => {
    if (!data?.details) return;
    deleteCompanyMutation.mutate(USERS.delete(staffId), {
      onSuccess: () => {
  ...
  return (
    <div className="w-full">
      <PageHeader title="جزئیات کارمند" showHomeIcon description="مشاهده کامل اطلاعات کارمند" />

      {isLoading && (
        <div className="mt-10 text-center text-gray-500">در حال بارگذاری اطلاعات کارمند...</div>
      )}

      {(!isLoading && (error || !details)) && (
        <div className="mt-10 text-center text-red-500">کارمند یافت نشد یا خطا در دریافت اطلاعات</div>
      )}

      {!isLoading && details && (
        <div className="space-y-6 mt-6">
          <div className="flex justify-end gap-3 mb-6">
            <DeleteButton type="button" size="md" onClick={() => setIsDeleteOpen(true)}>حذف کارمند</DeleteButton>
            <EditButton type="button" size="md" onClick={handleEdit}>ویرایش کارمند</EditButton>
          </div>
